window.Cloner = {
    ...
    async start(sourceId, targetId, token, options) {
        if (this.state.active) {
            if (window.UI) window.UI.addLog('Клонирование уже выполняется', 'warning');
            return false;
        }
        ...
        this.state = {
            ...
            options: options || { cloneRoles: true, cloneChannels: true, cloneIcon: true }
        };
        ...
        try {
            ...
            if (this.state.options.cloneRoles) await this.copyRoles();
            if (this.state.options.cloneChannels) await this.copyChannels();
            await this.finalize();
            return true;
        } catch (error) {
            if (error.message === 'CANCELLED') {
                if (window.UI) window.UI.addLog('Клонирование отменено', 'warning');
            } else {
                if (window.UI) {
                    window.UI.addLog(`Ошибка: ${error.message}`, 'error');
                    if (window.UI.showError) window.UI.showError(window.UI.elements.cloneError, error.message);
                }
            }
            return false;
        } finally {
    ...
    cancel() {
        if (this.state.active) {
            ...
            if (window.UI) window.UI.addLog('Отмена клонирования...', 'warning');
        }
    },
    ...
    async validatePermissions() {
        ...
        if (window.UI) window.UI.updateProgress(5, 'ПРОВЕРКА ПРАВ ДОСТУПА', 0, 0, 0, 0);
        ...
        if (window.UI) window.UI.addLog('Права администратора подтверждены', 'success');
    },

    async cleanTarget() {
        if (this.state.cancelled) throw new Error('CANCELLED');
        
        if (window.UI) window.UI.updateProgress(10, 'ОЧИСТКА КАНАЛОВ', 0, 0, 0, 0);
        ...
        try {
            ...
            for (let i = 0; i < channels.length; i++) {
                ...
                try {
                    ...
                    if (window.UI) window.UI.addLog(`Удален канал: ${channels[i].name}`, 'warning');
                } catch (e) {
                    if (window.UI) window.UI.addLog(`Не удалось удалить канал: ${channels[i].name}`, 'error');
                }
                await window.Utils.sleep(300);
                if (window.UI) window.UI.updateProgress(10 + (i / channels.length) * 10, 'ОЧИСТКА КАНАЛОВ', 0, 0, 0, 0);
            }
        } catch (e) {
            if (window.UI) window.UI.addLog(`Ошибка при очистке каналов: ${e.message}`, 'error');
        }
        
        if (window.UI) window.UI.updateProgress(20, 'ОЧИСТКА РОЛЕЙ', 0, 0, 0, 0);
        ...
        try {
            ...
            for (let i = 0; i < deletable.length; i++) {
                ...
                try {
                    ...
                    if (window.UI) window.UI.addLog(`Удалена роль: ${deletable[i].name}`, 'warning');
                } catch (e) {
                    if (window.UI) window.UI.addLog(`Не удалось удалить роль: ${deletable[i].name}`, 'error');
                }
                await window.Utils.sleep(300);
                if (window.UI) window.UI.updateProgress(20 + (i / deletable.length) * 10, 'ОЧИСТКА РОЛЕЙ', 0, 0, 0, 0);
            }
        } catch (e) {
            if (window.UI) window.UI.addLog(`Ошибка при очистке ролей: ${e.message}`, 'error');
        }
        
        if (window.UI) window.UI.updateProgress(30, 'ОЧИСТКА ЗАВЕРШЕНА', 0, 0, 0, 0);
    },

    async copyGuildInfo() {
        if (this.state.cancelled) throw new Error('CANCELLED');
        
        if (window.UI) window.UI.updateProgress(35, 'КОПИРОВАНИЕ ИНФОРМАЦИИ СЕРВЕРА', 0, 0, 0, 0);
        ...
        try {
            ...
            if (window.UI) window.UI.addLog(`Название скопировано: ${sourceGuild.name}`, 'success');
        } catch (e) {
            if (window.UI) window.UI.addLog(`Ошибка копирования названия: ${e.message}`, 'error');
        }
        
        if (this.state.options.cloneIcon) {
            try {
                const sourceGuild = await window.API.getGuild(this.state.token, this.state.sourceId);
                if (sourceGuild.icon) {
                    ...
                    if (iconData) {
                        ...
                        if (window.UI) window.UI.addLog('Иконка скопирована', 'success');
                    } else {
                        if (window.UI) window.UI.addLog('Не удалось скопировать иконку', 'warning');
                    }
                }
            } catch (e) {
                if (window.UI) window.UI.addLog(`Ошибка копирования иконки: ${e.message}`, 'warning');
            }
        }
        
        if (window.UI) window.UI.updateProgress(40, 'ИНФОРМАЦИЯ СКОПИРОВАНА', 0, 0, 0, 0);
    },

    async copyRoles() {
        if (this.state.cancelled) throw new Error('CANCELLED');
        
        if (window.UI) window.UI.updateProgress(45, 'ПОЛУЧЕНИЕ СПИСКА РОЛЕЙ', 0, 0, 0, 0);
        ...
        try {
            ...
            if (window.UI) window.UI.updateProgress(50, 'СОЗДАНИЕ РОЛЕЙ', 0, 0, total, 0);
            ...
            for (let i = 0; i < total; i++) {
                ...
                try {
                    ...
                    if (window.UI) window.UI.addLog(`Создана роль: ${role.name}`, 'success');
                } catch (e) {
                    if (window.UI) window.UI.addLog(`Ошибка создания роли ${role.name}: ${e.message}`, 'error');
                }
            ...
            if (window.UI) window.UI.addLog(`Ошибка при копировании ролей: ${e.message}`, 'error');
        }
        
        if (window.UI) {
            window.UI.updateProgress(70, 'РОЛИ СОЗДАНЫ', 100, 0, 0, 0);
    ...
    async copyChannels() {
        ...
        if (window.UI) window.UI.updateProgress(75, 'ПОЛУЧЕНИЕ СПИСКА КАНАЛОВ', 0, 0, 0, 0);
        ...
        try {
            ...
            if (window.UI) window.UI.updateProgress(80, 'СОЗДАНИЕ КАТЕГОРИЙ', 0, 0, 0, total);
            ...
            for (const cat of categories) {
                ...
                try {
                    ...
                    if (window.UI) window.UI.addLog(`Создана категория: ${cat.name}`, 'success');
                } catch (e) {
                    if (window.UI) window.UI.addLog(`Ошибка создания категории ${cat.name}: ${e.message}`, 'error');
                }
            ...
            if (window.UI) window.UI.updateProgress(85, 'СОЗДАНИЕ КАНАЛОВ', 0, 0, 0, total);
            ...
            for (const ch of others) {
                ...
                try {
                    ...
                    if (window.UI) window.UI.addLog(`Создан канал: ${ch.name}`, 'success');
                } catch (e) {
                    if (window.UI) window.UI.addLog(`Ошибка создания канала ${ch.name}: ${e.message}`, 'error');
                }
            ...
            if (window.UI) window.UI.addLog(`Ошибка при копировании каналов: ${e.message}`, 'error');
        }
        
        if (window.UI) {
            window.UI.updateProgress(100, 'КАНАЛЫ СОЗДАНЫ', 0, 100, 0, 0);
